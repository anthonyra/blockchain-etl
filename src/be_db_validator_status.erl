...
init(_) ->
    ...
    lager:info("Initializing validator status updater with an update rate: ~p", [RequestRate]),
    {ok, #state{
...
handle_info(check_status, State = #state{requests = Requests, max_peer_height = MaxPeerHeight0}) ->
    ...
    case RequestRate == State#state.request_rate of
        true ->
            ok;
        false ->
            lager:info("Validator status adjusting update rate to: ~p", [RequestRate])
    end,
    ...
    case AutoCatchup andalso DBHeight < max(MaxPeerHeight0, AssumedHeight) of
        ...
        false ->
            ...
            case DBHeight >= MaxPeerHeight0 + 1440 of
                true ->
                    _ = State#state{max_peer_height = be_utils:get_max_peer_height()};
                false ->
                    ok
            end,
    ...
    lager:info("Checking validator status again in ~p seconds", [BlockTime]),
    erlang:send_after(timer:seconds(BlockTime), self(), check_status),
